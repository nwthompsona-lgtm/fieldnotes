...
- ✅ **Phase 14e** — Watson eradication (§18; pilot feedback 8: "eliminate Watson anywhere in the defaults"). **Rename-safe seed**: `seedPilot` step 1 rewritten — an env-SET `PILOT_ORG_NAME`/`PILOT_PROJECT_NAME` is authoritative (a dashboard rename applies at the next boot, name-only update that PRESERVES an edited glossary/superName); with names unset the seed creates-if-missing under neutral names ("My Organization" / "Pilot Project") and never overwrites, so a rename done in-app or via a since-removed env sticks forever. `config.pilot.{orgName,projectName}` are now `string | undefined` — **no product-noun code defaults remain**. `PILOT_GLOSSARY` emptied (proper nouns are per-real-project data, not shipped defaults). Scrubbed: `render.yaml` + `render.dev.yaml` (name envs → `sync: false` + comment), both `.env` examples, SignupPage placeholder ("e.g. Meridian Builders"), the contracts glossary doc example; scripts (`verify-keys`/`tune`/`seed`) take the `?? 'Pilot Project'` fallback. Tests: 162/162 (+3: unset-names rename survival, env-name-wins with glossary preserved, fresh-DB neutral creation; fixtures de-Watsoned). Verified live across 5 boots on the hermetic rig: no-env boot preserves existing names; env-set boot renames org+project; later no-env boots never clobber back. (One live anomaly on record: hard-killing the dev server seconds after boot can lose pglite's unflushed write — test-rig artifact; Render/dev run Postgres.) *Dev dashboard still has the old Watson values in `PILOT_ORG_NAME`/`PILOT_PROJECT_NAME` — the user sets the real names there (or deletes the vars and renames in-app later); either way the seed now respects it.*

- ✅ **Phase 15a** — Workspace boot cache (`ddfbb08`). `WorkspaceProvider` boots from a token-bound localStorage snapshot (`{me, projects-per-org}`) and revalidates in the background — entering the shell from /capture (or a reload) is spinner-free; a failed refresh with a snapshot on screen logs a warning instead of the full-screen error (an expired session still signs out via the real 401). Sign-out clears the snapshot. In passing: the error-state Retry now refetches projects too (used to retry only /me, leaving projects permanently unfetched). Verified live: API server killed + reload → shell renders fully from the snapshot.
- ✅ **Phase 15b** — Exits from capture (`67791a2`). Capture Home header gained a "View reports" icon button → the picked project's reports list (spinner-free via 15a); ReportScreen header gained "View in reports" → the management review page; the install gate gained "Just here for the reports? Open the dashboard →" (the gate stays — durable iOS storage still requires the install for CAPTURE — but it no longer traps desktop/shared-link visitors). Verified live end-to-end in preview.
- ✅ **Phase 15c** — Legacy retirement, step 1: the `apps/capture` origin's final face is a **"FieldReport has moved"** screen (new `MoveScreen`, gate-first in App.tsx) linking to `WEB_BASE/capture` with re-install steps. It refuses to strand walk data: with a pending walk or observations on the active walk (per-origin IndexedDB can't move), it shows a warning card + **"Finish syncing here first"** which drops back into the old app; a clean device only ever sees the move screen. Build green. *Step 2 — user-gated, after the pilot confirms the re-install: retire the capture-dev Vercel deploy (or make it a redirect), shrink CORS to one origin per env, remove `apps/capture` from the build.*
- ✅ **Phase 15d** — Shared capture `<TopBar>` (commit B of topbar unification; feedback 5). New props-only primitive (`capture/components/TopBar.tsx` + `capbar-*` classes in capture.css, `.cap`-scoped by the build): glassy bar matching the management shell (translucent blur + hairline border), back/cancel-or-brandpin slot, title+subtitle-or-wordmark, right-controls slot, optional second row; 44px controls. Adopted by all five capture screens — Home (brand + View-reports/theme/status pill + project-pill row), CaptureFlow (cancel + step progress row), ReviewScreen, ReportScreen (back + AI chip + view-in-app), ProjectPickerScreen (back/brand). Verified live: Home + CaptureFlow bars render glassy with 44×44 controls; zero `.capbar` outside `.cap`; web build green.
...
Status: ✅ Phase 14 complete — 14a (`ac5a011`) · 14b (`7238c6b`) · 14c (`e910336`) · 14d (`6055f13`) · 14e (`2f49042`) · 14f resolved as keep-probe-until-Phase-15-verified (see above).
✅ Phase 15 complete — 15a (`ddfbb08`) · 15b (`67791a2`) · 15c step 1 · 15d (see §14.4).
User-gated leftovers: re-install the PWA from the new origin (the legacy origin now
shows the move screen), then say the word and 15c step 2 lands (retire capture-dev,
shrink CORS, drop apps/capture from the build) plus the probe-account cleanup (14f).
Phase 16 (bottom-tab shell) stays scoped-only until the pilot re-tests 14+15.
...
boot and stick — with 14e the seed never clobbers a rename back).
