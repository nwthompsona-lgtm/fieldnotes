...
- 🟡 **Phase 0** — Dev/staging environment. Repo side ✅ on `develop` (`render.dev.yaml`, `cors.allowedOrigins` in config, `.env.dev.example` ×3, `DEV_ENV.md`). Dashboards: Neon-dev DB ✅, R2-dev bucket ✅ (`fieldreport-dev`, endpoint `https://1aa5859de5ebf1cc2299702e8f2ee1a1.r2.cloudflarestorage.com`), **Render-dev server ✅ live at `https://fieldreport-server-dev.onrender.com`** (`/healthz` → `ok:true, storage:"s3", commit:38c8917`; STT/synthesis/email = mock, no AI/Resend keys yet). **Still pending:** 2× Vercel-dev projects (web-dev, capture-dev) → then the CORS loop (`CORS_ALLOWED_ORIGINS` + `WEB_BASE_URL` on Render) → record the two SPA URLs here + run the dev smoke pass.
