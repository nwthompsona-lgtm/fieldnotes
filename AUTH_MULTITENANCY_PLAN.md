...
- 🟡 **Phase 9 (F1)** — Web: auth screens + app shell/switchers + reports list. **Data layer done** (design-agnostic): `apps/web/src/session.ts` (localStorage bearer store + pub/sub, T-1) + `authApi.ts` (typed signup/login/logout/me, invite preview/accept, scoped reports list/read; 401 → session clear), bound to the frozen §4/§6 contracts. **Screens + app shell + reports-list UI BLOCKED** on the Claude Design zip → `.design-handoff/auth-ui/` (absent) + its README (which fills in the per-screen playbook). No throwaway UI built.
