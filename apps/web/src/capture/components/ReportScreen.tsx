...
export function ReportScreen({ reportId, online, onBack }: Props) {
  ...
  // "Handed off" — the actual send happens in the review surface's Send modal (§8);
  // since 13a that's an in-app navigation on the same origin.
  ...
  // Distribution lives in the review surface (§8: private per-recipient links, open
  // tracking, revoke). Since Phase 13a it's the SAME app on the SAME origin: finalize,
  // then react-router-navigate to the review page with the Send modal open — no popup,
  // no second login, no blocker to detect.
  async function send() {
    setFinalizing(true);
    setError(null);
    try {
      await ensureFinalized();
      setHandedOff(true);
      navigate(`${reviewUrl(reportId)}?send=1`);
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setFinalizing(false);
